...
local CLEAREDCODE = require("gamesense/http") or error("Failed to load http | https://gamesense.pub/forums/viewtopic.php?id=19253%22")
...
-- NON EXISTENT USELESS LINK https://pastebin.com/raw/iJPYhFqV changed to https://google.com
    CLEAREDCODE.get("https://google.com", function(success, response)
    if not success or response.status ~= 200 then
      return
    end
    if response.body then
        client.color_log(194, 194, 194,  "------------------------------------")
        client.color_log(194, 194, 194,  "~ Its so pasted i cant even look at it")
        client.color_log(194, 194, 194,  "---Loading Etterance.Tech [Beta]----")
        client.color_log(194, 194, 194,  "---------Load Success---------------")
        client.color_log(194, 194, 194,  "--Welcome to etterance.tech [beta]--")
        client.color_log(194, 194, 194,  "-----last Update: 11/04/2023--------")
...
local etterance = {
	luaenable = ui.new_checkbox('AA', 'Anti-aimbot angles', colours.default .. 'Etterance.' .. colours.green .. 'Tech [' .. colours.green .. build .. colours.green .. ']'),
	tabselect = ui.new_combobox('AA','Anti-aimbot angles', 'Tab selector', 'Main', 'Anti-Aim', 'Anti~Bruteforce', 'Other'),
	...
	main = {
		...
		main_label4 = ui.new_label('AA', 'Anti-aimbot angles', colours.default .. '      Etterance.Tech [BETA]'),
        main_space3 = ui.new_label('AA', 'Anti-aimbot angles', '    Author ~ '..colours.lightblue..'jaksford#0001'),
		main_space1 = ui.new_label('AA', 'Anti-aimbot angles', '     Last update 11/04/2023'),
	...
	visual = {
		...
		indicator_select = ui.new_multiselect('AA','Anti-aimbot angles', 'Indicators', 'Crosshair Indicators', 'Sigma Damage'),
		...
		notification_system = ui.new_multiselect('AA','Anti-aimbot angles', 'Logs Type', 'Logs Under Crossahir', 'Console Logs'),
...
local function set_menu_color()
    local r, g, b = ui.get(etterance.colorpicker)
...
for i=1, #aa_config do
    ...
    rage[i] = {
		...
		l_limit = ui.new_slider('AA', 'Anti-aimbot angles', 'Yaw Add Left', -180, 180, 0, true, '°'),
        r_limit = ui.new_slider('AA', 'Anti-aimbot angles', 'Yaw Add Right', -180, 180, 0, true, '°'),

        c_jitter = ui.new_combobox('aa', 'anti-aimbot angles', 'Jitter Type', {'Off','Offset','Center','Random'}),
        c_jitter_sli = ui.new_slider('aa', 'anti-aimbot angles', 'Jitter Amount', -180, 180, 0, true, '°', 1),
        c_body = ui.new_combobox('aa', 'anti-aimbot angles', 'Body Yaw', {'Off','Opposite','Jitter','Static'}),
        c_body_sli = ui.new_slider('aa', 'anti-aimbot angles', 'Body Yaw Amount', -180, 180, 0, true, '°', 1),
...
local function set_lua_menu()
    ...
    ui.set_visible(etterance.main.main_settings, select_main)
	ui.set_visible(etterance.keybinds.key_defensive, contains(ui.get(etterance.main.main_settings), 'Force Defensive') and select_main)
    ui.set_visible(etterance.keybinds.key_edge_yaw, contains(ui.get(etterance.main.main_settings), 'Edge-yaw') and select_main)
    ui.set_visible(etterance.keybinds.key_freestand, contains(ui.get(etterance.main.main_settings), 'Freestand') and select_main)
    local manual_aa = contains(ui.get(etterance.main.main_settings), 'Manual AA')
    ui.set_visible(etterance.keybinds.key_forward, manual_aa and select_main)
	ui.set_visible(etterance.keybinds.key_back, manual_aa and select_main)
    ui.set_visible(etterance.keybinds.key_left, manual_aa and select_main)
    ui.set_visible(etterance.keybinds.key_right, manual_aa and select_main)
    
	--------------
	-- ANTI-AIM
	--------------
    ui.set_visible(etterance.antiaim.aa_condition, select_aa)
    ui.set_visible(etterance.antiaim.c_pitch, select_aa)
    ui.set_visible(etterance.antiaim.c_yawbase, select_aa)

    ui.set_visible(etterance.antiaim.ab_enable, select_ab)
    ui.set_visible(etterance.antiaim.stg1_label, select_ab2)
    ui.set_visible(etterance.antiaim.stg1_body, select_ab2)
    ui.set_visible(etterance.antiaim.stg1_jit, select_ab2)

    ui.set_visible(etterance.antiaim.stg2_label, select_ab2)
    ui.set_visible(etterance.antiaim.stg2_body, select_ab2)
    ui.set_visible(etterance.antiaim.stg2_jit, select_ab2)

    ui.set_visible(etterance.antiaim.stg3_label, select_ab2)
    ui.set_visible(etterance.antiaim.stg3_body, select_ab2)
    ui.set_visible(etterance.antiaim.stg3_jit, select_ab2)
    ...
    ui.set_visible(etterance.visual.indicator_col, select_visuals and ui.get(etterance.visual.indicator_enable) and contains(ui.get(etterance.visual.indicator_select), 'Crosshair Indicators'))
    ui.set_visible(etterance.visual.indicator_col2, select_visuals and ui.get(etterance.visual.indicator_enable) and contains(ui.get(etterance.visual.indicator_select), 'Crosshair Indicators'))

	ui.set_visible(etterance.visual.enablenotifications, select_visuals)
	ui.set_visible(etterance.visual.notification_system, select_visuals and ui.get(etterance.visual.enablenotifications))
	ui.set_visible(etterance.visual.notification_col, select_visuals and ui.get(etterance.visual.enablenotifications))
	ui.set_visible(etterance.visual.notification_style, select_visuals and ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), 'Logs Under Crossahir'))
	ui.set_visible(etterance.visual.hitcol_label, select_visuals and ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), 'Logs Under Crossahir') and ui.get(etterance.visual.notification_style) == 'Dark mode')
	ui.set_visible(etterance.visual.misscol_label, select_visuals and ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), 'Logs Under Crossahir') and ui.get(etterance.visual.notification_style) == 'Dark mode')
	ui.set_visible(etterance.visual.hitcol, select_visuals and ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), 'Logs Under Crossahir') and ui.get(etterance.visual.notification_style) == 'Dark mode')
	ui.set_visible(etterance.visual.misscol, select_visuals and ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), 'Logs Under Crossahir') and ui.get(etterance.visual.notification_style) == 'Dark mode')

	ui.set_visible(etterance.visual.notification_style, false)
	ui.set_visible(etterance.visual.misscol, false)
	ui.set_visible(etterance.visual.misscol_label, false)
	ui.set_visible(etterance.visual.hitcol_label, false)

    ui.set_visible(etterance.misc.enable_clan_tag, select_misc)
...
local function handle_menu()
    local enabled = ui.get(etterance.luaenable) and ui.get(etterance.tabselect) == 'Anti-Aim'
...
client.set_event_callback('net_update_end', function()
	...
	if not ui.get(etterance.misc.enable_clan_tag) then
		client.set_clan_tag("")
	end

    if ui.get(etterance.misc.enable_clan_tag) then
...
ui.set_callback(etterance.misc.enable_clan_tag, function() client.set_clan_tag('\0') end)
---------------------------------------------
--[CLANTAG CHANGER]
---------------------------------------------
...
local function onscreen_elements()
    ...
    if contains(ui.get(etterance.visual.indicator_select), 'Sigma Damage') then
	...
	local indicatormaster = ui.get(etterance.luaenable) and ui.get(etterance.visual.indicator_enable)

	local scpd = entity.get_prop(lp, "m_bIsScoped")
    text_fade_animation(x + 2, y*2 -   15, 1, {r=r, g=g, b=b, a=255}, {r=r1, g=g1, b=b1, a=255}, "ETTERANCE.TECH ~ PREMIUM SCRIPT")
	...
	local centerfunc1 = renderer.measure_text('-', "ette")
    ...
    if indicatormaster and entity.is_alive(lp) then   
        if contains(ui.get(etterance.visual.indicator_select), 'Crosshair Indicators') then
                text_fade_animation(center[1] + 21, center[2] + 29, 1, {r=r, g=g, b=b, a=255}, {r=r1, g=g1, b=b1, a=255}, "ETTERANCE")
				--renderer.text(center[1] + 21, center[2] + 25, r, g, b, a,  'cb-', 0, 'etterance')
				...
				if ui.get(ref.dt[1]) and ui.get(ref.dt[2]) then
					...
					if doubletap_charged() then
						renderer.text(center[1] + 23, center[2] + 37 + (spacing * 8), r, g, b, a, "c-d", 0, "DT")

					else
						renderer.text(center[1] + 23, center[2] + 37 + (spacing * 8), 145, 145, 145, 255, "c-d", 0, "DT")
					...
					renderer.circle_outline(center[1] + 15, center[2] + 37 + (spacing * 8), 150, 150, 150, CHECKCHECK == 1 and 1 or 255, 2.9, 270, 5, 1)
					renderer.circle_outline(center[1] + 15, center[2] + 37 + (spacing * 8), r, g, b, CHECKCHECK == 1 and 1 or 255, 2.9, 270, CHECKCHECK, 1)

					spacing = spacing + 1
				end

				if ui.get(ref.os[2]) then
					renderer.text(center[1] + 19, center[2] + 37 + (spacing * 8), r, g, b, 255, "c-d", 0, "HS")
					spacing = spacing + 1

				end

				if ui.get(ref.forcebaim)then
					renderer.text(center[1] + 19, center[2] + 37 + (spacing * 8), 255, 102, 117, 255, "c-d", 0, "BAIM")
					spacing = spacing + 1
...
local get_hit_color = function()
	local r, g, b, a = ui.get(etterance.visual.hitcol)
	return r, g, b, a
end

local get_miss_color = function()
	local r, g, b, a = ui.get(etterance.visual.misscol)
	return r, g, b, a
end
...
table.insert(logs_txt, {
	...
	text = 'Welcome to Etterance.Tech [' .. build .. '] build loaded.',
	textdark = colours.grey ..'Welcome to ' .. noticol .. 'Etterance' .. colours.grey .. '.Tech' .. colours.grey .. ' [' .. noticol .. build .. colours.grey .. '] build loaded.',
...
local function aim_hit(e)
	...
	if ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), "Logs Under Crossahir") then
		table.insert(logs_txt, {		
			text = string.format(colours.grey .. '[' .. noticol .. 'etterance' .. colours.grey .. ']' .. noticol .. ' Hit ' .. colours.grey .. '%s\'s' .. noticol .. ' %s' .. colours.grey .. ' for ' .. noticol .. '%i' .. colours.grey .. '(%d) (' .. noticol .. '%i' .. colours.grey .. ' remaining) aimed=' .. noticol .. '%s' .. colours.grey .. '(' .. noticol .. '%s' .. colours.grey .. ') sp=' .. noticol .. '%s' .. colours.grey .. ' LC=' .. noticol .. '%s', name, hgroup, e.damage, on_fire_data.damage, health, aimed_hgroup, hitchance, on_fire_data.safe_point, on_fire_data.self_choke),
			textdark = string.format(colours.grey .. '[' .. noticol .. 'etterance' .. colours.grey .. ']' .. noticol .. ' Hit ' .. colours.grey .. '%s\'s' .. noticol .. ' %s' .. colours.grey .. ' for ' .. noticol .. '%i' .. colours.grey .. '(%d) (' .. noticol .. '%i' .. colours.grey .. ' remaining) aimed=' .. noticol .. '%s' .. colours.grey .. '(' .. noticol .. '%s' .. colours.grey .. ') sp=' .. noticol .. '%s' .. colours.grey .. ' LC=' .. noticol .. '%s', name, hgroup, e.damage, on_fire_data.damage, health, aimed_hgroup, hitchance, on_fire_data.safe_point, on_fire_data.self_choke),
	...
	if ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), "Console Logs") then
...
local function aim_miss(e)
	...
	if ui.get(etterance.visual.enablenotifications) and contains(ui.get(etterance.visual.notification_system), "Logs Under Crossahir") then
		table.insert(logs_txt, {
			...
			text = string.format(colours.grey .. '[etterance]' .. noticol .. ' Missed' .. colours.grey .. ' %s\'s' .. noticol .. ' %s'.. colours.grey ..'(' .. noticol .. '%i' .. colours.grey .. ')(' .. noticol .. '%s' .. colours.grey .. ') due to' .. noticol .. ' %s' .. colours.grey .. ':%.2f' .. noticol .. '°', name, hgroup, on_fire_data.damage, hitchance, e.reason, inaccuracy),
			textdark = string.format(colours.grey .. '[etterance]' .. noticol .. ' Missed' .. colours.grey .. ' %s\'s' .. noticol .. ' %s'.. colours.grey ..'(' .. noticol .. '%i' .. colours.grey .. ')(' .. noticol .. '%s' .. colours.grey .. ') due to' .. noticol .. ' %s' .. colours.grey .. ':%.2f' .. noticol .. '°', name, hgroup, on_fire_data.damage, hitchance, e.reason, inaccuracy),
	...
	if entity.is_alive(localplayer) then
		...
		for i, info in ipairs(logs_txt) do
			...
			if info.text ~= nil and info.text ~= "" then
				...
				local r, g, b, a = ui.get(etterance.visual.notification_col)

				if ui.get(etterance.visual.notification_style) == 'Top bar' then
					renderer.rectangle(
					...
					renderer.text(center[1] - 50 , add_y - 40, r, g, b, alpha, "", 0, "Etterance.Tech")
					renderer.text(center[1] - text_size_x / 2, add_y - 21, 250, 250, 250, alpha, '', 0, info.text)

				elseif ui.get(etterance.visual.notification_style) == 'Dark mode' then
...
----database.write('etterance_color', colours.lightblue)
end)
curtime = 0
client.set_event_callback('setup_command', function(e)
    ...
    if ui.get(rage[state].c_pitch_exp) then
        ...
        ui.set(ref.pitch, ui.get(etterance.antiaim.c_pitch))
    end
	...
	local force_def = ui.get(etterance.keybinds.key_defensive) and contains(ui.get(etterance.main.main_settings), 'Force Defensive')
    ...
    if contains(ui.get(etterance.main.main_settings), 'Anti~Backstab') then
...
client.set_event_callback("bullet_impact", function(e)
    ...
    if math.abs(delta_2d) <= 60 and globals.curtime() - lastmiss > 0.015 then
        ...
        table.insert(logs_txt, {
            text = noticol ..'[etterance.tech]'.. colours.grey ..' Anti~Bruteforce Angle Switched. Stage - '..tostring(stage),
            textdark = noticol ..'[etterance.tech]'.. colours.grey ..' Anti~Bruteforce Angle Switched. Stage - '..tostring(stage),
...
client.set_event_callback("setup_command", function(cmd)
    ...
    if stage == 1 then
        ui.set(ref.yawjitter[2], ui.get(etterance.antiaim.stg1_jit))
        ui.set(ref.bodyyaw[2], ui.get(etterance.antiaim.stg1_body))
    elseif stage == 2 then
        ui.set(ref.yawjitter[2], ui.get(etterance.antiaim.stg2_jit))
        ui.set(ref.bodyyaw[2], ui.get(etterance.antiaim.stg2_body))
    elseif stage == 3 then
        ui.set(ref.yawjitter[2], ui.get(etterance.antiaim.stg3_jit))
        ui.set(ref.bodyyaw[2], ui.get(etterance.antiaim.stg3_body))
    end
end)
...
local function init_callbacks()
    ui.set_callback(etterance.luaenable, handle_menu)
    ui.set_callback(etterance.antiaim.aa_condition, handle_menu)
    ui.set_callback(etterance.tabselect, handle_menu)
...
client.set_event_callback("pre_render", function()
	if not ui.get(etterance.luaenable) then return end

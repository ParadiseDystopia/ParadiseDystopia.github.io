...
local error_log, find_signature, timestamp = print, utils.find_pattern, function() return global_vars.realtime end -- man ev0lve unix_time() is in seconds
...
local vtable_bind = function(module, interface, index, typedef)
    local addr = ffi.cast("void***", utils.find_interface(module, interface)) or safe_error(interface .. " is nil.")
...
local pGetModuleHandle_sig = utils.find_pattern("engine.dll", " FF 15 ? ? ? ? 85 C0 74 0B") or error("Couldn't find signature #1")
local pGetProcAddress_sig = utils.find_pattern("engine.dll", " FF 15 ? ? ? ? A3 ? ? ? ? EB 05") or error("Couldn't find signature #2")
...
SteamAPI_RegisterCallResult = proc_bind("steam_api.dll","SteamAPI_RegisterCallResult","void(__cdecl*)(struct SteamAPI_callback_base *, uint64_t)")
SteamAPI_UnregisterCallResult = proc_bind("steam_api.dll", "SteamAPI_UnregisterCallResult", "void(__cdecl*)(struct SteamAPI_callback_base *, uint64_t)")

SteamAPI_RegisterCallback = proc_bind("steam_api.dll", "SteamAPI_RegisterCallback", "void(__cdecl*)(struct SteamAPI_callback_base *, int)")
SteamAPI_UnregisterCallback = proc_bind("steam_api.dll", "SteamAPI_UnregisterCallback", "void(__cdecl*)(struct SteamAPI_callback_base *)")

local SteamAPI_RunCallbacks = proc_bind("steam_api.dll", "SteamAPI_RunCallbacks", "void(__cdecl*)(void)")
...
-- all interfaces
--
local SteamAPI_GetHSteamUser = proc_bind("steam_api.dll","SteamAPI_GetHSteamUser","int(__cdecl*)()")
local SteamAPI_GetHSteamPipe = proc_bind("steam_api.dll","SteamAPI_GetHSteamPipe","int(__cdecl*)()")
...
index_funcs_extra.ISteamMatchmaking = function()
	local matchframework001 = utils.find_interface("matchmaking.dll", "MATCHFRAMEWORK_001")

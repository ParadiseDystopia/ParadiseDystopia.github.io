...
-- IWeaponSystem
local match = utils.find_pattern("client.dll", "8B 35 ? ? ? ? FF 10 0F B7 C0") or error("IWeaponSystem signature invalid")
...
for idx=1, 1000 do
    ...
    if res ~= nil then
        ...
        weapon.schema = utils.json_decode(js.get_weapon_info(idx))
...
local function weapons_get_by_entity(tbl, entindex)
    ...
    local idx = entindex:get_prop("m_iItemDefinitionIndex")
    return weapons[idx]
end
